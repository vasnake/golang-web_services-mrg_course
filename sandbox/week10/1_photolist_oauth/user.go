...
// Login: check login, password and create session if ok
...
// LoginOauth: oauth callback (redirect url) with 'code';
// create user from provided user info (or load from db using login), create session
func (uh *UserHandler) LoginOauth(w http.ResponseWriter, r *http.Request) {
	...
	pass := RandStringRunes(50)             // well, later we add option to login with password to oauth-created login (user have to change password)
	user, err := uh.createUser(login, pass) // create or get existing
...
// createUser: questionable logic: create or return existing
func (uh *UserHandler) createUser(login, passIn string) (*User, error) {
	...
	if err != sql.ErrNoRows {
		return user, errUserExists // you need user? you have one, existing
	}
