...
чтобы снаружи был веб-интерфейс (http модуль).
Тесты задают спеку на этот веб-интерфейс.
Теги и комментарии кодегена задают спеку реализации.
...
Пример тега к полю структуры:
Class    string `apivalidator:"enum=warrior|sorcerer|rouge,default=warrior"`

Комментарий-метка:
префикс `apigen:api` за которым следует json с полями url, auth, method (по умолчанию GET)
Пример:
