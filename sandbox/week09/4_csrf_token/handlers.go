...
type Storage interface {
	...
	Rate(uint32, int) error // new shit
}
...
// http handlers: list, upload, rate
type PhotolistHandler struct {
	St     Storage
	Tmpl   *template.Template
	Tokens TokenManager // new shit
}

// List: read user photos from db, create csrf token with session data, render template
func (h *PhotolistHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	err = h.Tmpl.ExecuteTemplate(w, "list",
		struct {
			Items     []*Photo // with rating and comments
			CSRFToken string
		}{
			Items:     items,
			CSRFToken: token, // <input type="hidden" value="{{.CSRFToken}}" name="csrf-token" />
		})
	if err != nil {
...
// Upload: check csrf token, save file to storage (metagata to db, file to disk)
...
// Rate: check csrf token, update pic rating in db
