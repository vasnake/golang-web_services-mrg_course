...
// new shit, another session implementation on jwt
// with DB
...
type SessionJWTVerClaims struct {
	...
	Ver    int32  `json:"ver,omitempty"` // new shit
	jwt.StandardClaims
}
...
// new token, set cookie (no db, WTF?)
...
// read cookie, check token, check user version from db; ; used in auth middleware
