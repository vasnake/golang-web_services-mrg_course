...
go run week02
```
[week 2 playground](./sandbox/week02/main.go)

### Методы обработки запросов (web) и плюсы неблокирующего подхода

Рассказ про:
- Асинхронное выполнение (помните AJAX?);
- Скорость потока данных процессор-кеш-память;
- Время на переключение контекста (выгрузка-загрузка регистров);
...
В целом: нужна асинхронность и параллельность, язык содержит средства получить это эффективно (горутины, каналы).
...
Чекает каналы без блокирования.
Что-то делает только если канал готов. Если все каналы остановлены, то действие "по умолчанию".

Поместить `select` в цикле, действие по умолчанию если ни один канал не работоспособен -- можно выходить из цикла.

Использование канала "команд" вместе с каналом "данных": если получили команду "на выход" -- завершаем работу и выходим.
...
Отмена по таймауту `ctx, cancelFunc := context.WithTimeout(context.Background(), workTime)`.
...
Профилирование покажет насколько падает скорость ...
...
- Бенч показал, что распаковка структуры через кодогенерацию вдвое быстрее рефлексии.
...
- Демонстрация медленности регулярных выражений на строках. На порядок медленнее (чем что?).
