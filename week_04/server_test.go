...
func CheckoutDummy(w http.ResponseWriter, r *http.Request) { // external service http handler mock
	key := r.FormValue("id")
	switch key {
...
func (c *Cart) Checkout(id string) (*CheckoutResult, error) { // some business logic for testing
...
func TestCartCheckout(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(CheckoutDummy)) // server mock
