...
Когда разработка и деплой "сервиса" происходят независимо от других частей системы,
...
Получилось (см. выше) неплохо, но бинарный Го-шный формат коммуникации (net/rpc) не самый гибкий и удобный.
...
`handouts/golang_web_services_2023-12-28.zip/7/microservices/grpc_2/`
...
Сервис в работе
[Код, домашки, литература](week_08/materials.zip)

UPD:
- `handouts/golang_web_services_2023-12-28.zip/8/`
- `handouts/golang_web_services_2024-04-26.zip/8/`

```s
pushd sandbox
mkdir -p week08 && pushd ./week08
go mod init week08
...
go run week08
```
[week 8 playground](./sandbox/week08/main.go) `GO_APP_SELECTOR=week08 gr`

### Конфигурирование сервиса - 1 (commandline args, cfg file, compile-time)

`handouts\golang_web_services_2023-12-28.zip\8\config\`
...
Можно использовать свой тип данных, для этого надо реализовать интерфейсный метод `func (x *MyType) Set(input string) error {...}`
и зацепить переменную за пакет `flag` в функции `init` модуля.
...
Тоже конфигурирование, компайл-тайм:
Можно при сборке бинарника указать (через флаги сборки `-ldflags`) набор значений для внутренних переменных.
Версия программы, бранч GIT, whatever (удобно для reproducible build)
`go run -ldflags="-X 'main.Version=$(git rev-parse HEAD)'" main.go`

Лично я предпочитаю env vars и command-line args (в командной строке можно переопределить енв).
В коде прописаны дефолты для конфига.

### Конфигурирование сервиса - 2 (hot-reload config, consul)

`handouts\golang_web_services_2023-12-28.zip\8\config\consul\`

Как в программе освежить конфиг без перезапуска программы?
hot-reload config, online-conf

- [consul_config](week_08/consul_config.go)

Два слоя: 1) слой online хранилища, где лежат данные конфига в виде key-value (http://localhost:8500/ui/dc1/kv);
2) слой использования и обновления конфига: конфиг как глобальная мапка в виде референса,
под защитой мютекса реф. переключается на новую версию мапки.
Новая версия создается при апдейте версии в хранилище.
Используется конфиг внутри обработчиков риквестов к сервису, в виде протаскивания текущего рефа в контекст запроса.
Как запрос отработал, так реф освободился и собрался GC.
...
Эта копия через контекст прокидывается по цепочке обработки запроса.
...
- [expvars](week_08/expvars.go) `handouts\golang_web_services_2023-12-28.zip\8\monitoring\expvars\`
- [graphite](week_08/graphite.go) `handouts\golang_web_services_2023-12-28.zip\8\monitoring\graphite\`

Пакет `expvar`. Позволяет зарегать дебаг обработчик как handler веб-страницы.
После чего по урл `/debug/vars` нам доступны некоторые показания.

Мы можем в них добавить свою стату: через создание мапки в пакете или через регистрацию функции в `expvar.Publish`.

Дергать руками такой урл для просмотра статы -- моветон (нет, это просто вариант поллинга статы).
Посмотрим, как использовать сервер статы -- graphite (вариант пушинга статы).
...
Как работать с фреймворком prometheus. `handouts\golang_web_services_2023-12-28.zip\8\monitoring\prometus\`
...
Этот хендлер выдаёт большую простыню разных метрик по процессу сервера, в формате нужном прометею.

Теперь надо настроить (yml) прометей, чтобы он ходил на эту страничку (делал поллинг).
У прометея есть свой дашборд, но лучше использовать графану.

ОК, сам сервер (процесс) работает. Как собрать метрики по бизнес-логике?
Используем структуры пакета `prometheus`: `NewSummaryVec`, `NewCounterVec`
для сбора количества вызовов своих процедур и времени, затраченного в них.
Заворачиваем процедуры в мидлварь (декораторы), где обновляем метрики.

эти метрики добавляются к простыне http://localhost:8080/metrics

P.S.
Код генерации метрик я воспроизвел в песочнице.
Запуск и настройку демона прометеуса (`docker run -p 9090:9090 prom/prometheus`) я похерил,
пусть у девопса голова болит, ничего интересного там нет.

prometheus dashboard http://localhost:9090/targets?search=
В качестве дашборда рекомендуют использовать графану. Красивое.
...
(смещение: номер байта начала значения от начала структуры; выравнивание: количество байт требуемое для последовательного доступа к значению).
...
Строка отличается (от слайса байт) отсутствием поля capacity в заголовке.

### Низкоуровневое программирование - 2 (C from Go)

- [cgo_basic](week_08/cgo_basic.go)
- [main.go](week_08/cgo_main.go); [main.c](week_08/cgo_main.c)
...
package `C`.
Это псевдопакет, весь код над строкой с `import "C"` будет скомпилирован GCC (теряется кросс-платформенность).
Аргументы и результат вызова сишной функции: надо конвертировать из/в Го.

Используя через такую связку внешние динамически линкуемые библиотеки (dll, so),
вы теряете преимущества статического бинарника, собираемого Го. Получаете потенциальный dll-hell.
...
Примеры адекватного применения cgo: запуск модели в tensorflow; запросы к sqlite.
...
- deprecated https://github.com/alecthomas/gometalinter
- recommended https://github.com/golangci/golangci-lint
```s
...
Корректность проверяется тестами.

Done. Поэлементрая итерация по таргет, считывая данные из источника.
Если что-то отсутствует или не того типа: ошибка.
