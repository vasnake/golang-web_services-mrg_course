...
JSON -- формат без схемы, тяжело декодировать но легко читать и лепить изменения.

protobuf -- кодогенерация, жёстко задана схема, бинарный: (номер поля и тип, длина данных, данные).
По файлу схемы генерится код для любой платформы `protoc --go_out=. *.proto`.
Поля нумеруются, поэтому можн добавлять ранее не задействованные номера => расширяемая схема.
Используется в gRPC.
