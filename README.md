...
Когда получен нужный результат, дергается функция завершения контекста и воркеры понимают (канал `ctx.Done()`), что пора выходить.

Отмена по таймауту. В канал `ctx.Done()` приходит сообщение по таймеру, не по ручному вызову функции завершения.

Контекст -- основной способ завершения вечных воркеров.
